...
class News extends Model
{
    ...
    {
        $html  = "<i class='fa fa-edit' onclick=layOpenView('/admin/news/newsedit/{$this->id}','100%','100%','信息编辑')></i>";
		if ($this->status) {
			$html .= "<i class='fa fa-level-down ml-15' onclick=layChangeStatus('/admin/news/newsstatus/{$this->id}/0','隐藏')></i>";
		} else {
			$html .= "<i class='fa fa-level-up ml-15' onclick=layChangeStatus('/admin/news/newsstatus/{$this->id}/1','展示')></i>";
		}
		$html .= "<i class='fa fa-trash-o ml-15' onclick=layListDel(this,'/admin/news/destroy','{$this->id}','tr')></i>";
		return $html;
    }
